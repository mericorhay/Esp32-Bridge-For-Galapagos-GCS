...
class UdpRelay {
  public:
    ...
    // Block up to `timeout_ms` for an incoming datagram into `out`.
    // Returns bytes received (0 == timeout). On return, `peer` holds the
    // sender address so the task can log it once.
    size_t recv(std::span<uint8_t> out, uint32_t timeout_ms) noexcept;
